...
from orchestrator_mvp.models import ReviewPolicyMode, RiskLevel, Tier
...
# ``ReviewPolicyMode`` from :mod:`orchestrator_mvp.review_adaptive`
# continue to work after the enum moved to :mod:`orchestrator_mvp.models`.
...
class ChangeClass(StrEnum):
    ...
    Distinct from review-effort levels in :mod:`orchestrator_mvp.review_policy`:
...
_SECURITY_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/sandbox/",
    r"^orchestrator_mvp/control_plane/",
    r"^orchestrator_mvp/authority\.py$",
    r"^orchestrator_mvp/publishing\.py$",
    r"^orchestrator_mvp/execution_failover\.py$",
    r"^orchestrator_mvp/ownership\.py$",
    r"^orchestrator_mvp/provenance\.py$",
)

_ACCEPTANCE_OR_GATE_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/accept\.py$",
    r"^orchestrator_mvp/gate_runner\.py$",
    r"^orchestrator_mvp/quota\.py$",
    r"^orchestrator_mvp/routing\.py$",
    r"^orchestrator_mvp/routing_decision\.py$",
    r"^orchestrator_mvp/routing_config\.py$",
    r"^orchestrator_mvp/process\.py$",
)

_PUBLICATION_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/publishing\.py$",
    r"^orchestrator_mvp/gateway/",
    r"^orchestrator_mvp/vcs/",
)

_RECOVERY_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/replay/",
    r"^orchestrator_mvp/persistence/",
    r"^orchestrator_mvp/db\.py$",
    r"^orchestrator_mvp/execution_failover\.py$",
)

_PLANNING_DOC_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/planning\.py$",
    r"^orchestrator_mvp/slicing\.py$",
    r"^orchestrator_mvp/project_scope/",
)
...
_ORDINARY_PRODUCTION_PATH_PATTERNS: Final[tuple[str, ...]] = (
    r"^orchestrator_mvp/classifier\.py$",
    r"^orchestrator_mvp/terminal\.py$",
    r"^orchestrator_mvp/templates/",
    r"^orchestrator_mvp/static/",
    r"^orchestrator_mvp/observability/",
    r"^orchestrator_mvp/telemetry\.py$",
)
...
def _detect_ownership_boundary(changed_paths: tuple[str, ...]) -> bool:
    ...
    Heuristic: a change that touches both ``orchestrator_mvp/<area>/`` and
    ``orchestrator_mvp/<other_area>/`` crosses an ownership boundary.  The
    ...
    for path in changed_paths:
        ...
        if path.startswith("tests/"):
            has_tests_root = True
        if path.startswith("orchestrator_mvp/"):
            parts = path[len("orchestrator_mvp/") :].split("/", 1)
...
def decide_review(
    *,
    ...
    weaken them.  ``enforce_hard_minimum`` would be the future extension
    seam; for C13-F the precedence above is the entire invariant.
    """
...
def load_project_review_policy(repo_path: Path | str) -> ReviewPolicyMode:
    ...
    :func:`orchestrator_mvp.gate_runner.load_project_gate_scratch_policy`
