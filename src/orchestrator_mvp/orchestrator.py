...
from orchestrator_mvp.authority import load_authority_settings
from orchestrator_mvp.autonomous_runtime import (
...
from orchestrator_mvp.config import (
    RunConfig,
    freeze_owner_binding_registry_payload,
    resolve_run_config,
)
from orchestrator_mvp.context_projection import (
    CHANGED_PATHS_MAX,
...
from orchestrator_mvp.control_plane import (
...
from orchestrator_mvp.dispatch import build_skip_payload, evaluate_eligibility
from orchestrator_mvp.dispatch import control_plane_decision as control_plane_decision
from orchestrator_mvp.execution_failover import FailoverEngine
from orchestrator_mvp.gateway import (
    AffinityHint,
...
from orchestrator_mvp.git import (
...
from orchestrator_mvp.observability import new_dispatch_evidence, update_dispatch_from_result
...
from orchestrator_mvp.planning import (
    build_context_capsule,
    mark_step,
    persist_plan,
    ready_dependents,
)
from orchestrator_mvp.process import (
...
from orchestrator_mvp.project_scope.graph import build_project_graph
from orchestrator_mvp.project_scope.persistence import (
    freeze_run_scope,
    load_scope_bundle,
    scope_section_for_run,
)
from orchestrator_mvp.project_scope.resolver import resolve_package_scope, resolve_scope
...
from orchestrator_mvp.quota import apply_quota_to_chain  # noqa: F401
from orchestrator_mvp.routing import resolve_candidate_chain
...
from orchestrator_mvp.routing_decision import (
    TierDecision,
    choose_tier,
...
from orchestrator_mvp.slicing import resolve_work_packages
from orchestrator_mvp.telemetry import persist_worker_usage
from orchestrator_mvp.terminal import get_transcript_path
from orchestrator_mvp.tooling import (
    FrozenToolContract,
    ToolRegistry,
    freeze_tool_contract,
    tool_context_metrics,
)
from orchestrator_mvp.tooling.invocation import read_telemetry
from orchestrator_mvp.validation_policy import (
...
class _ReadinessAdmission(Protocol):
    ...
    :class:`orchestrator_mvp.model_access.ReadinessService`, which
...
class Orchestrator:
    ...
    def _publish_gate_passed_candidate(
        ...
        *,
        ...
        from orchestrator_mvp.publishing import (
        ...
        *,
        ...
        if persisted is not None:
            try:
                ...
                validate_invariants = (
                    __import__(
                        "orchestrator_mvp.review_adaptive",
        ...
        if candidate_sha and base_commit and candidate_sha != base_commit:
            try:
                from orchestrator_mvp.review_adaptive import diff_changed_paths
        ...
        if auto_tier_unresolvable and not decision.independent_review_required:
            ...
            from orchestrator_mvp.review_adaptive import ReviewRequirementSource
    ...
    def recover_run(
        ...
        *,
        ...
        :meth:`orchestrator_mvp.supervisor.RunSupervisor.ensure_recovery_running`).
        """
        ...
        if persisted_run is not None and persisted_run.config_json:
            ...
            if config is None:
                from orchestrator_mvp.config import reconstruct_run_config
    ...
    def run_autonomous_project(
        ...
        *,
        ...
        try:
            ...
            from orchestrator_mvp.accept import AcceptEngine
    ...
    def _run_core(
        ...
        *,
        ...
        if run_id is not None:
            ...
            # autonomous worker dispatch; this correction concerns the ordinary
        ...
        # retain the exact Slice 3A lifecycle/events and do not incur planning
        # model calls; the deterministic sizing assessment never reads adapters.
        plan_steps: list[Any] = []
        ...
        try:
            ...
            while True:
                ...
                # `python -m orchestrator_mvp.devscope scope` rule)
                ...
                if gateway_outcome is not None and (
                    ...
                    worker_res = WorkerResult(
                        ...
                        model=candidate.model,
                        status=AttemptStatus.FAILED,
                        summary="C10 gateway blocked dispatch",
                        stdout="",
                        stderr="",
                        error=(f"C10 gateway execution blocked: {actual_reason}"),
                        outcome=DispatchOutcome.PROVIDER_OR_RUNTIME_FAILURE,
                    )
                    ...
                    if is_clean_pass:
                        ...
                        if not decision.independent_review_required:
                            ...
                            try:
                                mark_phase_not_applicable(
                                    ...
                                    notes=(
                                        "C14-C: REVIEW did not apply to this "
                                        "candidate -- the authoritative C13-F "
                                        "risk-adaptive decision resolved "
                        ...
                        if decision.independent_review_required:
                                ...
                                while True:
                                    ...
                                    if (
                                        ...
                                        w_res = WorkerResult(
                                            ...
                                            model=cand_model,
                                            status=AttemptStatus.FAILED,
                                            summary="C10 gateway blocked repair dispatch",
                                            stdout="",
                                            stderr="",
                                            error=(
                                                f"C10 gateway execution blocked: {actual_reason_rep}"  # noqa: E501
                                            ),
                                            outcome=DispatchOutcome.PROVIDER_OR_RUNTIME_FAILURE,
                                        )
                            ...
                            if reviews and reviews[-1].verdict == ReviewVerdict.PASS_WITH_BACKLOG:
                                from orchestrator_mvp.backlog import (
