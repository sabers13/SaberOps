...
Execution requires an exact :class:`~orchestrator_mvp.control_plane.bindings.ExecutionBinding`
whose complete identity (``binding_id`` / ``provider`` / ``model`` /
``backend`` / ``profile_id`` / ``quota_pool_id``) survives a real
:func:`~orchestrator_mvp.control_plane.bindings.resolve_execution_binding`
...
* This is a **control-plane** (C11-B) document.  It is not Project truth and
  ...
  :mod:`orchestrator_mvp.routing_config`.
...
from orchestrator_mvp.control_plane.capabilities import Capability, ProviderTransport
from orchestrator_mvp.control_plane.orch_binding import OrchBindingPolicy
from orchestrator_mvp.models import ReasoningEffort
