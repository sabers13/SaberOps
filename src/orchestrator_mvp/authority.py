...
table below, so a future UI can bind a dropdown straight to this module
...
class AuthorityPolicy:
    ...
    # which stays reserved for future accepted-target/release publication.
...
def get_authority_config_path() -> Path:
    ...
    This mirrors :func:`orchestrator_mvp.routing_config.get_user_routing_path`
...
def load_authority_policy() -> AuthorityPolicy:
    ...
    Next phase: `orchestrator_mvp.cli.handle_accept` is the one trivial
    acceptance entry point where reading ``policy.auto_accept`` is clearly
    appropriate — it is the single place a candidate is integrated, so an
    automatic-acceptance path can consult this without spreading authority
    checks through the engine.  This slice deliberately stops at the setting
    and the agent bridge; no command consumes the policy yet.
    """
