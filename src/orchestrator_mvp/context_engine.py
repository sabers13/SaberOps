...
seam for future prompt caching.  It extends (does not replace) the
...
from orchestrator_mvp.project_ledger.observation import RepositoryObservation
from orchestrator_mvp.project_ledger.store import ProjectLedger
...
class ContextSection:
    ...
    same digest; this is the property that lets future backends
    detect unchanged sections for prompt-cache reuse.
...
class ContextDelta:
    ...
    exposes which sections are unchanged / changed / new so future
    backends can retransmit only the deltas where the provider
